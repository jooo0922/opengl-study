...
// 노란색으로 칠해주는 프래그먼트 쉐이더 소스코드 추가
const char* fragmentShaderSource2 = "#version 330 core\n" // 쉐이더 버전 명시
"out vec4 FragColor;\n" // out 키워드로 vec4 타입의 프래그먼트 쉐이더 출력 변수 선언
"void main()\n"
"{\n"
"	FragColor = vec4(1.0f, 1.0f, 0.0f, 1.0f);\n" // 프래그먼트 쉐이더의 최종 출력값을 vec4 타입의 노랑색 값으로 할당
"}\n\0"; // NULL 종료 문자 '\0' 로 문자열의 끝부분임을 표시
...
int main()
{
	...
	unsigned int fragmentShader1; // OpenGL 쉐이더 객체(object)의 참조 id를 저장할 변수
	fragmentShader1 = glCreateShader(GL_FRAGMENT_SHADER); // OpenGL 쉐이더 객체(object) 생성
	glShaderSource(fragmentShader1, 1, &fragmentShaderSource1, NULL); // 생성된 프래그먼트 쉐이더 객체(object)에 쉐이더 소스코드 문자열 붙임
	glCompileShader(fragmentShader1); // 프래그먼트 쉐이더 객체(object)를 런타임에 동적으로 컴파일

	glGetShaderiv(fragmentShader1, GL_COMPILE_STATUS, &success); // 생성된 쉐이더 객체에 대한 정보를 질의할 수 있는 쿼리 함수
	if (!success)
	{
		// 컴파일 실패 시, 에러 메시지 출력
		glGetShaderInfoLog(fragmentShader1, 512, NULL, infoLog); // 문자열 배열 변수 infoLog 에 프래그먼트 쉐이더 객체(object)에 대한 InfoLog 를 최대 512자까지 저장
		std::cout << "ERROR::SHADER::FRAGMENT::COMPILATION_FAILED\n" << infoLog << std::endl;
	}

	// 프래그먼트 쉐이더 객체 생성 및 컴파일
	unsigned int fragmentShader2; // OpenGL 쉐이더 객체(object)의 참조 id를 저장할 변수
	fragmentShader2 = glCreateShader(GL_FRAGMENT_SHADER); // OpenGL 쉐이더 객체(object) 생성
	glShaderSource(fragmentShader2, 1, &fragmentShaderSource2, NULL); // 생성된 프래그먼트 쉐이더 객체(object)에 쉐이더 소스코드 문자열 붙임
	glCompileShader(fragmentShader2); // 프래그먼트 쉐이더 객체(object)를 런타임에 동적으로 컴파일

	glGetShaderiv(fragmentShader2, GL_COMPILE_STATUS, &success); // 생성된 쉐이더 객체에 대한 정보를 질의할 수 있는 쿼리 함수
	if (!success)
	{
		// 컴파일 실패 시, 에러 메시지 출력
		glGetShaderInfoLog(fragmentShader2, 512, NULL, infoLog); // 문자열 배열 변수 infoLog 에 프래그먼트 쉐이더 객체(object)에 대한 InfoLog 를 최대 512자까지 저장
		std::cout << "ERROR::SHADER::FRAGMENT2::COMPILATION_FAILED\n" << infoLog << std::endl;
	}

	// 생성된 여러 쉐이더 객체 연결 > 쉐이더 프로그램 객체 생성
	unsigned int shaderProgram1 = glCreateProgram(); // OpenGL 쉐이더 프로그램 객체(object)의 참조 id를 저장할 변수
	glAttachShader(shaderProgram1, vertexShader); // 그래픽 파이프라인의 입출력 순서에 따라 쉐이더를 프로그램 객체에 붙여줘야 함. (즉, 버텍스 쉐이더 -> 프래그먼트 쉐이더 순!)
	glAttachShader(shaderProgram1, fragmentShader1);
	glLinkProgram(shaderProgram1); // 쉐이더 프로그램에 붙여진 쉐이더 객체를 연결

	glGetProgramiv(shaderProgram1, GL_LINK_STATUS, &success); // 생성된 프로그램 객체에 대한 정보를 질의하는 쿼리 함수
	if (!success)
	{
		// 쉐이더 프로그램 linking 실패 시, 에러 메시지 출력
		glGetProgramInfoLog(shaderProgram1, 512, NULL, infoLog); // 문자열 배열 변수 infoLog 에 쉐이더 프로그램 객체(object)에 대한 InfoLog 를 최대 512자까지 저장
	...
	glGetProgramiv(shaderProgram2, GL_LINK_STATUS, &success); // 생성된 프로그램 객체에 대한 정보를 질의하는 쿼리 함수
	if (!success)
	{
		// 쉐이더 프로그램 linking 실패 시, 에러 메시지 출력
		glGetProgramInfoLog(shaderProgram2, 512, NULL, infoLog); // 문자열 배열 변수 infoLog 에 쉐이더 프로그램 객체(object)에 대한 InfoLog 를 최대 512자까지 저장
		std::cout << "ERROR::SHADER::PROGRAM::LINKING_FAILED\n" << infoLog << std::endl;
	}
	...
	// 두 개의 삼각형 정점 데이터를 각각의 버퍼로 생성하여 관리
	float vertices1[] = {
	...
	*/
	unsigned int VBOs[2], VAOs[2]; // VBO, VAO 객체(object) 참조 id 를 각각 두 개씩 저장할 변수
	glGenVertexArrays(2, VAOs); // 두 개의 VAO(Vertex Array Object) 객체 생성
	glGenBuffers(2, VBOs); // 두 개의 VBO(Vertex Buffer Object) 객체 생성 > VBO 객체를 만들어서 정점 데이터를 GPU 로 전송해야 한 번에 많은 양의 정점 데이터를 전송할 수 있음!

	/* 첫 번째 VAO, VBO 객체 바인딩 및 데이터 저장 */
	glBindVertexArray(VAOs[0]); // VAO 객체 먼저 컨텍스트에 바인딩(연결)함. > 그래야 재사용할 여러 개의 VBO 객체들 및 설정 상태를 VAO 에 바인딩된 VAO 에 저장할 수 있음.

	glBindBuffer(GL_ARRAY_BUFFER, VBOs[0]); // OpenGL 컨텍스트 중에서, VBO 객체는 GL_ARRAY_BUFFER 타입의 버퍼 유형 상태에 바인딩되어야 함.
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices1), vertices1, GL_STATIC_DRAW); // 실제 정점 데이터를 생성 및 OpenGL 컨텍스트에 바인딩된 VBO 객체에 덮어씀.
	...
	/* 두 번째 VAO, VBO 객체 바인딩 및 데이터 저장 */
	glBindVertexArray(VAOs[1]); // 두 번째 VAO 객체를 컨텍스트에 바인딩(연결)함.

	glBindBuffer(GL_ARRAY_BUFFER, VBOs[1]); // 두 번째 VBO 객체 바인딩
	glBufferData(GL_ARRAY_BUFFER, sizeof(vertices2), vertices2, GL_STATIC_DRAW); // 두 번째 버텍스 데이터를 두 번째 VBO 객체에 덮어쓰기

	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, 3 * sizeof(float), (void*)0); // 두 번째 VBO 객체에 쓰여진 정점 데이터 해석 방식 설정
	glEnableVertexAttribArray(0); // 원래 버텍스 쉐이더의 모든 location 의 attribute 변수들은 사용 못하도록 디폴트 설정이 되어있음. > 그 중에서 0번 location 변수만 사용하도록 활성화한 것!
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// ...
		// 삼각형 그리기 명령 실행
		glUseProgram(shaderProgram1); // 미리 생성 및 linking 해둔 쉐이더 프로그램 객체를 사용하여 그리도록 명령
		glBindVertexArray(VAOs[0]); // 미리 생성한 VAO 객체를 바인딩하여, 해당 객체에 저장된 VBO 객체와 설정대로 그리도록 명령
		glDrawArrays(GL_TRIANGLES, 0, 3); // 주황색 삼각형 먼저 그리기

		glUseProgram(shaderProgram2); // 노랑색으로 칠해주는 쉐이더로 linking 한 쉐이더 프로그램 객체로 바인딩 교체 
		glBindVertexArray(VAOs[1]); // 두 번째 VAO 객체 바인딩
		glDrawArrays(GL_TRIANGLES, 0, 3); // 노랑색 삼각형 그리기
	...
	glDeleteVertexArrays(2, VAOs); // 여러 개의 VAO 객체를 제거할 경우, 여러 개의 참조 id 가 담긴 배열 변수를 그대로 넘겨줌. (& 앰퍼샌드로 주소값 넘기기X)
	glDeleteBuffers(2, VBOs); // 여러 개의 VBO 객체를 제거할 경우, 여러 개의 참조 id 가 담긴 배열 변수를 그대로 넘겨줌. (& 앰퍼샌드로 주소값 넘기기X)
