...
int main()
{
	...
	// 5. stride. 정점 데이터 세트 사이의 메모리 간격. 각 정점은 현재 3개의 위치 데이터와 3개의 노멀 데이터를 가져다 쓰므로, 
	// <실수형 리터럴의 메모리 크기 * 6> 이 정점 데이터 세트 간 메모리 간격
	...
	// VBO 에 쓰여진 정점 데이터 중에서, 노멀 데이터 해석 방식 설정
	// attribute 변수의 location 은 1로 지정하고 (버텍스 쉐이더에서 aNormal 을 1로 지정할 것임.), 
	// 정점 버퍼에서 데이터 시작 위치 offset 은 첫 3개의 위치 데이터를 건너뛰어야 하므로, 
	// float 타입 크기(4 bytes) * 3 = 12 bytes 로 설정
