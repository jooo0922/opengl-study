...
void main() {
  /* ambient 성분 계산 */
  vec3 ambient = light.ambient * material.ambient; // (ambient 강도 * 물체가 ambient 에 대해 반사하는 색상) 으로 최종 ambient 성분값 계산 (원래는 각 성분마다 색상을 별도 지정할 수 있어야 함.)
  ...
  vec3 diffuse = light.diffuse * (diff * material.diffuse); // diffuse 조명 색상 * (diffuse 조도 * 물체가 diffuse 에 대해 반사하는 색상) 으로 최종 diffuse 성분값 계산
  ...
  vec3 specular = light.specular * (spec * material.specular); // (specular 조도 * 물체가 specular 에 대해 반사하는 색상) * 조명색상 으로 specular 성분값 계산
