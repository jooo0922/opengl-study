...
// 버텍스 쉐이더에서 전송된 보간 변수
in vec3 Normal; // 뷰 공간 노멀벡터
in vec3 FragPos; // 뷰 공간 프래그먼트 위치값
in vec3 LightPos; // 뷰 공간 광원 위치
...
void main() {
  ...
  vec3 viewDir = normalize(-FragPos); // 뷰 벡터 -> 뷰 공간 기준 조명계산 시, 뷰 벡터는 <영벡터 - 프래그먼트 뷰좌표> 와 동일함!
