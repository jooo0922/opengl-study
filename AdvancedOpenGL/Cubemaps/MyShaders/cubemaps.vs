...
layout(location = 1) in vec3 aNormal; // Assimp 로 로드한 모델링 파일에 포함된 버텍스 노멀 데이터

// 프래그먼트 쉐이더로 보간하여 전송할 월드공간 위치 및 노멀 데이터
out vec3 Normal;
out vec3 Position;
...
void main() {
  // 정점 노멀벡터를 월드공간 노멀벡터로 변환 > 프래그먼트 쉐이더로 보간 전송
  Normal = mat3(transpose(inverse(model))) * aNormal; 

  // 정점 오브젝트 공간 좌표에 모델행렬만 곱해서 월드공간 좌표로 변환 > 프래그먼트 쉐이더로 보간 전송
...
/*
  노멀 행렬 (Normal Matrix) 
  (https://learnopengl.com/Lighting/Basic-Lighting 참고)
